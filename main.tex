...
\maketitle
\begin{abstract}
The implementation of novel early-phase dose-finding designs in clinical practice has been slow, and the classical yet inefficient 3+3 design remains a common choice. Most comparative studies focus on Phase I operating characteristics, leaving open the question of whether differences in dose-finding behaviors meaningfully translate into downstream impact on drug development success.

We propose a simulation framework to evaluate how commonly used dose-finding designs (3+3, BOIN, CRM, BOIN12, and EffTox) impact the probability of significant results in Phase III trials. We consider both chemotherapy-like profiles, where the classical monotonicity assumption about toxicity and efficacy holds, and immunotherapy-like drug profiles where the efficacy typically plateaus before reaching the maximum tolerable dose (MTD). Different toxicity control rules are implemented in Phase II and III to reflect varying extents of safety monitoring in practice. Furthermore, we redesign the development process of a targeted therapy, Imatinib, in patients with advanced gastrointestinal stromal tumour (GIST), to illustrate the proposed simulation pipeline. 

For chemotherapy-like drug profiles, BOIN and CRM substantially outperform 3+3 across a range of late-phase toxicity control settings. In contrast, for immunotherapy-like drug profiles, the advantage of more efficient designs over 3+3 is compromised, particularly under stricter toxicity control. Bivariate designs, such as EffTox and BOIN12, yield a more robust overall success rate in immunotherapy-like drug profiles, but provide limited benefit for chemotherapy-like drug profiles. Increasing the Phase I sample size beyond 30 patients does not meaningfully improve the overall probability of success.

The downstream performance of dose-finding methods heavily depends on the underlying drug profiles. Designs that perform well in identifying the MTD lead to a poor late-phase success rate when the dose-efficacy relationship has an early plateau in immunotherapy-like profiles. These findings highlight the importance of understanding the potential shape of the candidate drug profile from pre-clinical studies when designing early-phase trials. 

\end{abstract}
\section{Introduction}
\medskip
\textit{I have restructured the Introdunction: Clinical problem - MTD paradigm - evolution of MTD designs - OBD paradigm - literature gap}
\medskip

Early-phase dose-finding trials play a critical role in characterizing the safety and dosing of promising therapeutic candidates. The overall success rate of drug development remains low, with approximately $90\%$ of drug candidates eventually failing after advancing to the Phase I stage \parencite{drug_failure}. One of the major reasons for the high attrition rate is the lack of clinical efficacy and unmanageable toxicity, which can often be attributed to suboptimal dose selections in early-phase trials \parencite{drug_failure}. Such failures are especially costly in oncology drug development, where a larger proportion of unsuccessful drug candidates progressed to resource-demanding late-stage trials before being abandoned \parencite{dimasi2007economics}. 

Traditionally, dose selection has been guided by the toxicity-efficacy relationship observed in chemotherapies, where the monotonicity assumption typically holds. Specifically, the efficacy is assumed to increase monotonically with dose, making it reasonable to select the highest dose below the toxicity threshold, known as the maximum tolerable dose (MTD). Trial designs therefore only consider toxicity endpoints and escalate doses until reaching the MTD. One such classical design is the 3+3 design, which has been used in oncology Phase I trials for decades. The popularity of the 3+3 design comes primarily from its simplicity. As a rule-based design, it follows straightforward escalation and de-escalation rules and requires minimal prior knowledge \parencite{rule_based_design}. 

However, rule-based designs only use data from the immediately previous cohort to guide dose decisions, discarding potentially valuable information \parencite{clertant2022early}. To address this limitation, model-based designs such as the Continuous Reassessment Method (CRM) were developed to use all accumulated data and explicitly estimate the underlying dose-toxicity relationship \parencite{CRM,EWOC,review_model_based_design}. While being more efficient, these approaches are also more complex. Model-assisted designs, such as the Bayesian Optimal Interval (BOIN) design \parencite{BOIN}, strike a balance between statistical efficiency and operational complexity. They use all data from the current dose level for dose escalation and all data at the end of the trial for estimating the MTD. Meanwhile, they maintain implementation simplicity comparable to rule-based designs \parencite{zhou2018comparative}. 

In recent years, the landscape of oncology drug development has been transformed by the emergence of novel treatments such as molecular-target agents and immunotherapies. For these drugs, efficacy often plateaus before reaching the MTD \parencite{agrawal_nivolumab_2016,bekaii-saab_regorafenib_2019,fujimoto_phase_2017}, motivating consideration of both toxicity and efficacy to identify an optimal biological dose (OBD) \parencite{MTD_OBD, OBD_clinical_efficacy}. This shift of focus has stimulated the development of bivariate designs that jointly model toxicity and efficacy outcomes, including model-based approaches such as EffTox and model-assisted approaches such as BOIN12 \parencite{BOIN12,EffTox}. 

Despite extensive methodological development \parencite{kessels2026knowledge}, the adoption of novel statistical designs in drug development has been slow: the 3+3 design remains a common choice in approximately 74\% of oncology Phase I trials \parencite{review_phase1}. Model-assisted and model-based designs encounter practical complexities of implementation: they often require additional design specifications, such as thresholds for dose admissibility and prior assumptions about the dose-response relationship, as well as more involvement of statistical expertise \parencite{clertant2022early}. Additional barriers include the lack of practical implementation tools, limited real-world experience, and underdeveloped guidance for protocol development \parencite{petroni2017implementation}. Therefore, suboptimal dose selection may arise from both the use of inefficient designs, such as the 3+3, and the failure to incorporate efficacy information for modern immunotherapy-type agents. 

Beyond these implementation challenges, an important gap remains, considering how dose-finding designs are typically evaluated in the literature. Most comparative studies of early-phase trial designs focus exclusively on Phase I operating characteristics, such as the probability of correctly identifying the MTD and the probability of overdosing \parencite{horton2017performance,iasonos2008comprehensive,van2016performance,zhou2018comparative}. While these metrics are informative, they do not capture the ultimate goal of drug development, which is to bring safe and efficacious treatments to patients as efficiently as possible. The extent to which performance differences across dose-finding designs translate into meaningful downstream outcomes, particularly the probability of success in late-phase trials, remains largely unexamined.

Some studies have attempted to evaluate the downstream consequences of dose selection in Phase I. Existing simulation frameworks, however, remain limited in several important aspects. First, they generally simplify or omit safety monitoring in later phases. For example, \textcite{conaway2019impact} assessed excess toxicity using a single binomial test at the end of Phase III, which is likely to have very lower power. Furthermore, toxicity is usually continuously monitored in clinical practice and may lead to early trial suspension. Similarly, \textcite{ruppert2018overall} did not incorporate explicit toxicity control rules in later phases, potentially favoring more aggressive dose-finding strategies by allowing overly toxic doses to proceed without consequence.

Second, previous studies considered only a restricted range of drug profiles. The scenarios investigated by \textcite{conaway2019impact} were characterized by relatively low toxicity, limiting the ability to evaluate the aggressive dose-escalation behavior of CRM, which is known to have a tendency toward overdosing \parencite{EWOC}. Consequently, it remains unclear whether the reported superiority of CRM generalizes to settings with steeper toxicity profiles. Although \textcite{ruppert2018overall} evaluated a broader range of drug profiles, both studies focused primarily on univariate dose-finding methods that rely exclusively on toxicity outcomes. This focus may become increasingly less relevant for current practice in oncology, given the growing emphasis towards OBD selection in the development of non-cytotoxic agents \parencite{araujo2023oncology}. 

In this paper, we propose a comprehensive simulation framework to evaluate the impact of representative dose-finding methods on the success rate of Phase III trials. We assess the robustness of each method across various drug profiles featured by both chemotherapies, where the dose-efficacy relationship parallels the dose-toxicity relationship, and immunotherapies, where the efficacy plateaus before the MTD. We further consider different toxicity control rules in late-phase trials to reflect varying extents of safety monitoring in practice. We additionally compare univariate designs to their bivariate extensions to quantify the potential benefit and limitations of incorporating early efficacy data in dose escalation. Finally, we redesign the development process of a targeted therapy, Imatinib, in patients with advanced gastrointestinal stromal tumour (GIST), to illustrate the proposed simulation pipeline.

\section{Methods}
\subsection{Simulation framework}
Figure \ref{simulation-framework} illustrates the overall workflow of the proposed simulation framework. Depending on the stage of the trial, commonly used patient-level outcomes are generated from corresponding drug profiles. Candidate dose-finding designs are implemented in Phase I to select a recommended Phase II dose (RP2D). The selected RP2D then determines the efficacy and toxicity profile carried forward into subsequent trial phases, thereby linking Phase I dose selection directly to downstream development outcomes. The remainder of the drug development process (Phase II and Phase III designs) is kept fixed such that the effect of dose selection can be isolated. Both late phases incorporate a potential trial termination rule due to excess toxicity, mimicking the safety monitoring that occurs in practice. The overall success rate is then evaluated based on the probability of significant results in Phase III trials among all simulated trial sequences.
\begin{figure}[ht]
\centering
...
For all dose-finding designs except 3+3, the primary comparison used a Phase I sample size of 30 subjects, reflecting common practice in oncology Phase I trials \parencite{li_phase_2024}. We also investigated the impact of sample size allocation between early-phase and late-phase trials by increasing the Phase I sample size to 40, 50, and 60 subjects while deducting the corresponding amount of subjects in Phase III to maintain a fixed total sample size. 
...
\toprule
Method & Design class & Endpoint & Key tuning parameters$^\dagger$ & Admissible criteria \\
\midrule
...
\makecell[tl]{$N=30,40,50,60$ \\ Cohort size = 3 \\ Target DLT = 30\%} &
...
CRM & Model-based & DLT &
\makecell[tl]{$N=30,40,50,60$ \\ Cohort size = 1 \\ Target DLT = 30\%}  &
Terminate if the first dose indicates de-escalation; doses with $\ge 0.95$ posterior probability of DLT $>$ target are inadmissible \\
...
$N=30,40,50,60$\\
Cohort size: 3\\
Toxicity/efficacy limits: \\
$\phi_T=0.35$, $\phi_E=0.5$\\
Utility table: \\
$u_{1(\text{no tox, eff})}=100$,\\
$u_{2(\text{no tox, no eff})}=40$,\\
$u_{3(\text{tox, eff})}=60$,\\
$u_{4(\text{tox, no eff})}=0$ 
}
&
...
Standard designs were implemented in Phase II and III to isolate the effect of dose selection in Phase I (Table \ref{phase23-designs}). In Phase II, a Simon two-stage minimax design was used to evaluate the TRR of the selected RP2D \parencite{simon2stage}. The type I error was controlled at 10\% (two-sided) and the power was targeted to be 80\%. We assumed a TRR of 35\% for the standard-of-care and a target TRR of 50\% for the new drug. A total of 49 subjects were planned, with 31 subjects enrolled in the first stage. The trial continues to the second stage only if more than 10 subjects achieve a tumor response in the first stage. At the end of the trial, efficacy is declared if more than 21 subjects demonstrate tumor response. In Phase III, a two-arm randomized controlled trial (RCT) was used to compare the OS of the selected RP2D against the standard-of-care. The type I error was controlled at 5\% (two-sided) and the power was targeted to be 80\%. We assumed the OS of the standard-of-care followed an exponential distribution with a median OS of 14 months, both for the sample size calculation and for data generation. The treatment is expected to provide a median OS benefit of 6 extra months, which corresponds to a hazard ratio of 0.7. The maximum follow-up time was set to 60 months, with an expected accrual duration of 12 months. A total of 218 subjects were planned, with 109 subjects in each arm.

For both Phase II and III, we considered excess toxicity control rules to represent the safety monitoring that occurs in practice. In real life, safety monitoring of clinical trials is complicated and involves joint discussion and decision-making from the Data Safety Monitoring Board, which is hard to be realistically implemented in simulation. We implemented a trial termination rule if the accumulating DLT rate of the treatment arm during the trial exceeds the toxicity suspension threshold (Table \ref{phase23-designs}), with an observation window of 20 patients for Phase II and 30 patients for Phase III. Though being much simplified, this effectively represents different extents of toxicity tolerance in the drug development process, with smaller thresholds reflecting more conservative safety monitoring practices.  
...
To illustrate the proposed simulation framework, we redesigned the clinical development process of Imatinib in patients with advanced gastrointestinal stromal tumour (GIST). The challenge in creating real-world drug profiles is that in later phases of drug development, only one or two doses have been investigated. Therefore, there is limited information on response rates and survival times of doses that were not carried on to Phase II and Phase III. However, the PK/PD data from the Phase II trial of Imatinib were reported \parencite{demetri2009imatinib}. Due to heterogeneity in individual absorption/clearance rate, patient cohorts that receive the same dose will still have different serum concentrations of the drug. As a result, we can use drug exposure as a surrogate for drug dose. 

The data generation was divided into two steps. We first generated the exposure data for the patient cohort receiving different doses based on their relevant pharmacological characteristics (Figure \ref{Dist-Imatinib-Cmin}). Then we mapped the simulated exposure to clinical outcomes via the constructed exposure-toxicity, exposure-response, and exposure-survival curves derived from clinical reports \parencite{demetri2009imatinib}. Because of the variation in Imatinib exposure, the resulting dose-toxicity, dose-response and dose-survival curves are stochastic rather than fixed. Figure \ref{Imatinib-dose-profile} illustrates 100 simulated Imatinib profiles. Details of the data generation process are described in the Appendix. 

We then applied the simulation framework to evaluate the PoS under different Phase I dose-finding designs and late-phase toxicity control rules. The Phase II and Phase III designs were reconstructed based on the actual designs used in the clinical development of Imatinib. The original Phase II trial of Imatinib was an RCT comparing the 400 mg dose to the 600 mg dose \parencite{demetri2002efficacy}. Its statistical plan specified that recruitment would continue only if at least 3 of 18 treated patients in each arm achieved a response. We adapted this design to a single-arm Simon two-stage design targeting 80\% power and 60\% TRR. It requires 6 responses out of 16 subjects in the first stage to continue and a total of 14 responses out of 28 in total to declare efficacy. The original Phase III trial of Imatinib was an RCT comparing the 400 mg dose to the 800 mg dose \parencite{blanke2008phase}. A median progression-free survival (PFS) of 18 months was assumed for the lower-dose arm, with a target hazard ratio of 1.4, corresponding to an expected median PFS of 25 months for the higher-dose arm. The target power was 92\%. The accrual was estimated to take 2 years, plus an extra year for follow-up. We calculated the sample size accordingly and we ended up with 310 patients per arm. The toxicity suspension thresholds were set to be the same as those used in the main analysis for comparability.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/Imatinib_dose_profile.png}
    \caption{Dose-DLT rate, dose-TRR, and dose-median PFS curves for 100 simulated Imatinib profiles. The vertical dashed lines indicate the target DLT of $30\%$ in the Phase I trial; target TRR of $60\%$ in the Phase II trial; target median PFS of 25 months in the Phase III trial.}
...
For bivariate designs that jointly consider toxicity and efficacy data to identify the OBD, the dose selection varies across drug profiles. For chemotherapy-like profiles, bivariate designs are generally less efficient in identifying the MTD compared to their univariate counterparts as the additional efficacy information provides limited benefit under monotonic dose-efficacy relationships while increasing model complexity and estimation uncertainty in small-sample settings. For immunotherapy-like profiles, bivariate designs have decent performance in identifying the OBD: BOIN12 identifies the OBD at a rate of 36\% and 40\% for the early and late plateau profiles, respectively; EffTox identifies the OBD at a rate of 33\% and 45\% for the early and late plateau profiles, respectively. Moreover, in occasions when the true OBD is not selected, BOIN12 tends to recommend one dose lower while EffTox one dose higher. Hence, EffTox exhibits more aggressive dose selection behavior relative to BOIN12.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{plots/phase1_dose_selection.png}
    \caption{Probability of selecting each dose as the RP2D across 1000 simulated Phase I trials under N=30 for univariate designs (top) and bivariate designs (bottom). NA means the Phase I trial is terminated early and no RP2D is selected. For univariate designs and bivariate designs in chemotherapy-like profiles, the target dose is the MTD (dose 5) in green; for bivariate designs in immunotherapy-like profiles, the target dose is the OBD (dose 3 for early plateau and dose 4 for late plateau) in blue.}
    \label{phase1-dose-selection}
\end{figure}

Increasing the Phase I sample size from 30 to 60 subjects improves the probability of selecting the MTD for BOIN and CRM, which increases from 69\% to 76\% and from 74\% to 88\%, respectively (Figure \ref{phase1-dose-selection-N40}, Figure \ref{phase1-dose-selection-N50}, Figure \ref{phase1-dose-selection-N60}). For bivariate designs in immunotherapy-like profiles, the probability of selecting the OBD increases only for EffTox, and the biggest gain is observed from 30 to 40 subjects. For BOIN12, the probability of selecting the OBD remains similar across different sample sizes, which may suggest that it is not consistent in estimating the OBD. Noticeably, the probability of no RP2D being selected increases as the Phase I sample size increases for BOIN12, especially in chemotherapy-like profiles. This is because when the efficacy curve is relatively flat at lower doses, BOIN12 tends to stop for futility before exploring higher doses. 
...
Figure \ref{plot-vary-ph1-N} shows the PoS when the Phase I sample size is increased from 30 to 60 patients, with a corresponding reduction in the Phase III sample size to keep the total sample size fixed. As the 3+3 design has no pre-specified sample size, its results remain unchanged across the four column panels. Overall, increasing the Phase I sample size provides little improvement in PoS across the evaluated dose-finding methods. The main exception is the chemo-like profile with a steep efficacy curve, where BOIN12 and EffTox benefit from the additional Phase I data. However, in this scenario, BOIN and CRM already achieve higher PoS with the standard Phase I sample size of 30, so that the additional investment in Phase I offers limited practical benefit.

For immunotherapy-like profiles, increasing the Phase I sample size helps to improve the robustness of EffTox by better targeting the OBD. BOIN12 is less sensitive to sample size changes, which is consistent with its more conservative dose selection behavior. The limited gain in PoS from increasing Phase I sample size is unlikely due to the fact that the Phase III sample size is reduced accordingly. Results from a sensitivity analysis with fixed Phase III sample size at 218 subjects (Figure \ref{plot-vary-ph1-N-fixed-ph3}) show similar trends, suggesting that the choice of the dose-finding method plays a bigger role than sample size in Phase I during the drug development process.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{plots/vary_ph1_N.png}
    \caption{Probability of overall success (PoS) when Phase I sample size is 30, 40, 50, and 60. The corresponding Phase III sample size is 218, 208, 198, and 188. The error bars represent 95\% Monte Carlo confidence intervals.}
    \label{plot-vary-ph1-N}
\end{figure}

\subsection{Sensitivity analysis results}
For BOIN12, as $u_2$, the utility value for outcome combination (No Toxicity, No Response), increases, the design becomes more conservative with increasing probability of selecting one dose below the OBD than one dose above it. But the probability of selecting the OBD is relatively stable across different values of $u_2$ (Figure \ref{plot-sens-BOIN12-RP2D}). Under liberal toxicity control, the PoS shows moderate variation across different values of $u_2$, except in the chemotherapy-like profile with a steep efficacy increase, where the PoS is nearly unchanged. Immuno-agents often show efficacy at doses much lower than the MTD. Hence it makes sense to set $u_2 < u_3$, i.e., $u_2 < 50$ to weigh efficacy more than toxicity. For this range of utility specification, the PoS is relatively robust with nearly overlapping curves (Figure \ref{plot-sens-BOIN12-PoS}). 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{plots/phase1_dose_selection_ph1_N40.png}
    \caption{Probability of selecting each dose as the RP2D across 1000 simulated Phase I trials under N=40 for univariate designs (top) and bivariate designs (bottom). NA means the Phase I trial is terminated early and no RP2D is selected. For univariate designs and bivariate designs in chemotherapy-like profiles, the target dose is the MTD (dose 5) in green; for bivariate designs in immunotherapy-like profiles, the target dose is the OBD (dose 3 for early plateau and dose 4 for late plateau) in blue.}
    \label{phase1-dose-selection-N40}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{plots/phase1_dose_selection_ph1_N50.png}
    \caption{Probability of selecting each dose as the RP2D across 1000 simulated Phase I trials under N=50 for univariate designs (top) and bivariate designs (bottom). NA means the Phase I trial is terminated early and no RP2D is selected. For univariate designs and bivariate designs in chemotherapy-like profiles, the target dose is the MTD (dose 5) in green; for bivariate designs in immunotherapy-like profiles, the target dose is the OBD (dose 3 for early plateau and dose 4 for late plateau) in blue.}
    \label{phase1-dose-selection-N50}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{plots/phase1_dose_selection_ph1_N60.png}
    \caption{Probability of selecting each dose as the RP2D across 1000 simulated Phase I trials under N=60 for univariate designs (top) and bivariate designs (bottom). NA means the Phase I trial is terminated early and no RP2D is selected. For univariate designs and bivariate designs in chemotherapy-like profiles, the target dose is the MTD (dose 5) in green; for bivariate designs in immunotherapy-like profiles, the target dose is the OBD (dose 3 for early plateau and dose 4 for late plateau) in blue.}
    \label{phase1-dose-selection-N60}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{plots/vary_ph1_N_fixed_ph3.png}
    \caption{Probability of overall success (PoS) when Phase I sample size is 30, 40, 50, and 60. The corresponding Phase III sample size is fixed at 218. The error bars represent 95\% Monte Carlo confidence intervals.}
    \label{plot-vary-ph1-N-fixed-ph3}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/plot_Imatinib_ph1.png}
    \caption{Probability of selecting each dose as the RP2D for Imatinib across 1000 simulated Phase I trials for univariate designs (top) and bivariate designs (bottom). NA means the Phase I trial is terminated early and no RP2D is selected.}
    \label{plot-Imatinib-ph1}
\end{figure}
