...
\maketitle
\begin{abstract}
A considerable amount of effort has been dedicated to improving the design of early-phase dose-finding trials in oncology. However, the implementation of novel designs in clinical practice has been slow, and the classical yet inefficient 3+3 design remains a common choice. Most comparative studies of early-phase trial designs focus on Phase I operating characteristics, leaving open the question of whether differences in dose selection meaningfully translate into downstream impact on drug development success.

We propose a simulation framework for the overall drug development process. We use it to evaluate how commonly used dose-finding designs (3+3, BOIN, and CRM) impact the probability of significant results in Phase III trials. We consider both chemotherapy-like profiles, where the classical monotonicity assumption about toxicity and efficacy holds, and immunotherapy-like drug profiles where the efficacy typically plateaus before reaching the maximum tolerable dose (MTD). Different toxicity control rules are implemented in Phase II and III to reflect varying extents of safety monitoring in practice. We further compare univariate designs considering only toxicity data to their bivariate extensions (BOIN12, EffTox) to investigate the potential benefits of incorporating early efficacy data. Finally, we redesign the development process of a targeted therapy, Imatinib, in patients with advanced gastrointestinal stromal tumour (GIST), to illustrate the proposed simulation pipeline. 

For chemotherapy-like drug profiles, BOIN and CRM substantially outperform 3+3 across a range of late-phase toxicity control settings. In contrast, for immunotherapy-like drug profiles, the advantage of more efficient designs over 3+3 is compromised, particularly under stricter toxicity control. Bivariate designs, such as EffTox and BOIN12, yield a more robust overall success rate in immunotherapy-like drug profiles, but provide limited benefit for chemotherapy-like drug profiles. BOIN12 is less sensitive to utility specification than EffTox with a Phase I sample size of 30 patients, although this difference diminishes with larger Phase I sample sizes. Increasing the Phase I sample size beyond 30 patients does not meaningfully improve the overall probability of success.

The downstream performance of dose-finding methods heavily depends on the underlying drug profiles. Designs that perform well in identifying the MTD lead to a poor late-phase success rate when the dose-efficacy relationship has an early plateau in immunotherapy-like profiles. Utilizing efficacy information in Phase I does not necessarily guarantee higher overall success rates, likely because the limited sample size and follow-up time provide insufficient information to reliably estimate both efficacy and toxicity. These findings highlight the importance of understanding the potential shape of the candidate drug profile from pre-clinical studies when designing early-phase trials. The proposed simulation framework complements traditional Phase I evaluations by providing a more comprehensive perspective on drug development, serving as a planning tool to assess how drug profile assumptions and safety monitoring strategies interact with early-phase design choices to influence the overall success rate.
...
Increasing the Phase I sample size from 30 to 60 subjects improves the probability of selecting the MTD for BOIN and CRM, which increases from 69\% to 76\% and from 74\% to 88\%, respectively (\ref{phase1-dose-selection-N40,phase1-dose-selection-N50,phase1-dose-selection-N60}). For bivariate designs in immunotherapy-like profiles, the probability of selecting the OBD increases only for EffTox, and the biggest gain is observed from 30 to 40 subjects. For BOIN12, the probability of selecting the OBD remains similar across different sample sizes, which may suggest that it is not consistent in estimating the OBD. Noticeably, the probability of no RP2D being selected increases as the Phase I sample size increases for BOIN12, especially in chemotherapy-like profiles. This is because when the efficacy curve is relatively flat at lower doses, BOIN12 tends to stop for futility before exploring higher doses. 
